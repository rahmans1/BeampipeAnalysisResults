...
T.Add(Form("/scratch/j/jmammei/rahmans/scratch/targetStudy/shortened_newcol12_uv_JLAB_75/beam_20mil_%s/beam_%d.root",conf.Data(), i));
}
Int_t nEvents=T.GetEntries();
Double_t weight= (1.0/nEvents)*(1.0/(1.602*1e-13));
std::cout<< "Analyzing " << nEvents << " events in "<< conf.Data() << std::endl;

TFile f(Form("/scratch/j/jmammei/rahmans/root/beampipeStudy/%s_%d_%s_%s.root",conf.Data(),nEvents,particle.Data(),energy.Data()), "RECREATE");
